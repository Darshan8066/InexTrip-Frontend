...
const TripCard= ({ trip, isAuthenticated, currentUser, onBookClick, onRefresh }) => {
  const isFav = currentUser?.favourites?.includes(trip.id);
  ...
  const toggleFav = (e) => {
    ...
    if (currentUser) {
      storageService.toggleFavourite(currentUser.id, trip.id);
