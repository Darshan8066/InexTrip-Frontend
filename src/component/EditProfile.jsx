...
const EditProfile = ({user, onLogout }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {         // When form submitted
            try {

                const res = await updateProfile(values);          // Call backend API to update user
                setAuth(sessionStorage.getItem("token"), res.user);        // Save updated user in localStorage
    ...
    const handleFileChange = async (e) => {
        ...
        try {
            ...
            const uploadRes = await axios.post("/upload/upload", formData);             // Upload to Cloudinary
